...
# This test is used to make sure that if periods to breaks is set to true
# that list pairs still works
assert kwic("Hello buddy. hello there buddy", periodsToBreaks = True, ignoreWords =[], listPairs = True) == [(['buddy', 'hello', 'there'], 1),
(['buddy.', 'Hello'], 0), (['Hello', 'buddy.'], 0), (['hello', 'there', 'buddy'], 1),
 (['there', 'buddy', 'hello'], 1), [(('hello', 'buddy'), 2)]]


assert kwic("Hello goodbye, buddy \nHello Goodbye buddy \nHello buddy, there \n Hello there!"
			,periodsToBreaks = False,ignoreWords = [], listPairs = True) == [(['buddy', 'Hello', 'goodbye,'], 0),
			(['buddy', 'Hello', 'Goodbye'], 1), (['buddy,', 'there', 'Hello'], 2), (['Goodbye', 'buddy', 'Hello'], 1),
			(['goodbye,', 'buddy', 'Hello'], 0), (['Hello', 'goodbye,', 'buddy'], 0), (['Hello', 'Goodbye', 'buddy'], 1),
			(['Hello', 'buddy,', 'there'], 2), (['Hello', 'there!'], 3), (['there', 'Hello', 'buddy,'], 2),
			(['there!', 'Hello'], 3), [(('goodbye', 'buddy'), 2), (('hello', 'buddy'), 3),
			(('hello', 'goodbye'), 2), (('hello', 'there'), 2)]]
