...
module.exports = class crud {
   ...
   async addToSet(searchObj, array, pushingElement) {
      return await self.db.updateOne(searchObj, { $addToSet: { [array]: pushingElement } })
   }

   async pullFromSet(searchObj, array, value) {
      return await self.db.updateOne(searchObj, { $pull: { [array]: { 'id': value } } })
   }

   async removeFromSet(searchObj, pullOp) {
      return await self.db.update(searchObj, pullOp);
   }
}
