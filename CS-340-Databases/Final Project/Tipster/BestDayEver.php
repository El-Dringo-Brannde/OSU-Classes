...
<?php
  ...
  $sql = "SELECT Date,TipSum,DeliveryCount from Shift where TipSum = (SELECT Max(TipSum) from Shift) and Username = '$user'";
...
//==================================================================
  ...
  $sql = "SELECT count(`DeliveryID`) from `Delivery` where `Date` = '$BestDay' and `Username` = '$user'";
  ...
  echo "</ul> </h4>";
       ...
       $query = "SELECT Location.Address, Location.Street, Location.APT, Delivery.Time,Delivery.OrderTotal,Delivery.Tip$ from Delivery, Location where Delivery.DeliveryID = Location.DeliveryID and Delivery.Username = '$user' and Delivery.Date = '$BestDay'";
         ...
         echo "<h1>Deliveries Taken by $user on $BestDay </h1>";
         ...
         $sql = "SELECT min(`Time`) from `Delivery` where `Date` = '$BestDay' and `Username` = '$user'";
         $result = $connection->query($sql);
         $fields_num = mysqli_num_fields($result);
        ...
        $sql = "SELECT max(`Time`) from `Delivery` where `Date` = '$BestDay' and `Username` = '$user'";
        $result = $connection->query($sql);
        $fields_num = mysqli_num_fields($result);
        ...
        $sql = "SELECT sum(`Tip$`) from `Delivery` where `Date` = '$BestDay' and `Username` = '$user'";
        $result = $connection->query($sql);
        $fields_num = mysqli_num_fields($result);
        for ($i = 0; $i < $fields_num; ++$i)
            $field = mysqli_fetch_field($result);
          while ($row = mysqli_fetch_row($result))
           ...
           $sql = "SELECT mid(`Time`,1,2), `Tip$` from Delivery where `date` = '$BestDay' and `Username` = '$user'";
           $result = $connection->query($sql);
           $fields_num = mysqli_num_fields($result);
           $EmptyArray = array();
           while ($row = mysqli_fetch_row($result)){
           ...
           $TextHoursWorked = "On $BestDay you worked $FixedHoursWorked hours";
