...
<?php
  ...
  $sql = "SELECT Date,TipSum,DeliveryCount from Shift where TipSum = (SELECT Min(TipSum) from Shift) and Username = '$user'";
...
//==================================================================
  ...
  $sql = "SELECT count(`DeliveryID`) from `Delivery` where `Date` = '$WorstDay' and `Username` = '$user'";
  ...
  echo "</ul> </h4>";
       ...
       $query = "SELECT Location.Address, Location.Street, Location.APT, Delivery.Time,Delivery.OrderTotal,Delivery.Tip$ from Delivery, Location where Delivery.DeliveryID = Location.DeliveryID and Delivery.Username = '$user' and Delivery.Date = '$WorstDay'";
         ...
         echo "<h1>Deliveries Taken by $user on $WorstDay </h1>";
         ...
         $sql = "SELECT min(`Time`) from `Delivery` where `Date` = '$WorstDay' and `Username` = '$user'";
         $result = $connection->query($sql);
         $fields_num = mysqli_num_fields($result);
        ...
        $sql = "SELECT max(`Time`) from `Delivery` where `Date` = '$WorstDay' and `Username` = '$user'";
        $result = $connection->query($sql);
        $fields_num = mysqli_num_fields($result);
        ...
        $sql = "SELECT sum(`Tip$`) from `Delivery` where `Date` = '$WorstDay' and `Username` = '$user'";
        $result = $connection->query($sql);
        $fields_num = mysqli_num_fields($result);
        for ($i = 0; $i < $fields_num; ++$i)
            $field = mysqli_fetch_field($result);
          while ($row = mysqli_fetch_row($result))
           ...
           $sql = "SELECT mid(`Time`,1,2), `Tip$` from Delivery where `date` = '$WorstDay' and `Username` = '$user'";
           $result = $connection->query($sql);
           $fields_num = mysqli_num_fields($result);
           $EmptyArray = array();
           while ($row = mysqli_fetch_row($result)){
           ...
           $TextHoursWorked = "On $WorstDay you worked $FixedHoursWorked hours";
