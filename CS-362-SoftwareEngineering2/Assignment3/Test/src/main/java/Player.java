...
class Player{
	...
	public String getName(){return this.Player_Name;}
	...
	private void handle_CutPurse(Player p1,Player p2){
		Player p; 
		boolean added = false; 
		System.out.println(this.getName() + this.Player_Hand.toString());
		for (int j = 0; j < PlayDominion.Players.size(); j++){
			p = PlayDominion.Players.get(j);
			if (p.getName() != this.getName()){
				for(int i = 0; i < p.Player_Hand.size(); i++){
					if (p.Player_Hand.get(i).getName() == "Copper"){
						p.Player_Hand.remove(i); //get rid of a copper
						added = true;  
						break; 
					}
				}
			...
			if (added == false)
				System.out.println(p.getName() + "No copper in hand found! \n" + p.toString());
			added = false; 
		}
	}
	...
	private void handle_CouncilRoom(Player p1, Player p2){
		...
		for (int i = 0; i < 4; i++)
			this.Draw(0);		
		for (int j = 0; j < PlayDominion.Players.size(); j++){
			Player p = PlayDominion.Players.get(j);
			if (p.getName() != this.getName()){
				if (p.Player_Deck.size() < 2)
					p.getNewDeck();
				p.Draw(0);
			}
		}
