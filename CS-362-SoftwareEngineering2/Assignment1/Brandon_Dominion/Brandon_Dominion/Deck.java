...
//----------------------------Action Cards ----------------	
	...
	private static List<Cards> curseCards = new ArrayList<Cards>();  
	
	public Boolean checkforEmpties(){
		int EmptyCount = 0;
		
		for(int i = 0; i < ActionCardDeck.size(); i++){
			if (ActionCardDeck.get(i).size() == 0)
				EmptyCount++;	
		}
		if (EmptyCount >= 3) 
			return true;
		
		// If the provinces are all gone
		if (VictoryCardDeck.get(2).size() == 0)
			return true; 
	...
	public int get_Gold_Cost(int i){ return ActionCardDeck.get(i).get(i).getCost(); }
	...
	public static void Build_Money_Victory(){
		...
		for (int i = 0; i < 10; i++){
			EstateCard = new Cards(Victory_Cards[3],V_Card_Cost[3],V_Card_Points[3],"Victory",false);
			VictoryCardDeck.get(3).add(EstateCard);
		}
		...
		for (int i = 0; i < 10; i++){
			...
			curseCards.add(Curse);
		}
	//	for (int i = 0; i < MoneyCardDeck.size(); i++)
		//	System.out.println(MoneyCardDeck.get(i).toString());
	}
