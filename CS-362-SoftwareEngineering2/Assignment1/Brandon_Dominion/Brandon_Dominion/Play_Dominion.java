...
class PlayDominion{
	...
	// Need to make have the end game scenarios
	public static void Play_Game(){
		//System.out.println(p1.get_PlayHand().toString());
		//System.out.println(p2.toString());
		//if (AllDecks.checkforEmpties() == true)
			//quit();
		...
		//REMEMBER TO GET RID OF CARDS AFTER PLAY
		
		//p2.Play_Card(Deck.getActionDeck().get(12).get(0),p1,p2);
		p1.Get_Player_Stats();
		p2.Get_Player_Stats();
	...
	// MAKE SURE THAT IF THEY DON"T HAVE ANYTHING TO PLAY 
	...
	//	System.out.println("Player 2: GOLD IN HAND " + Player_2_Gold);
		...
		if (Action_Card_Present == -1)
			...
			Cards PlayMe = p2.get_PlayHand().get(Action_Card_Present);
		...
		do {
			...
			CheckifPileIsEmpty(randomVal);
			if (Deck.getActionDeck().get(randomVal).size() != 0 ){
				if (Player_2_Gold < Deck.getActionDeck().get(randomVal).get(0).getCost()){
			//		System.out.println("Not enough Gold");
					...
					Cards Bought = Deck.getActionDeck().get(randomVal).remove(0);
			...
			//	System.out.println(Bought.toString());
					...
					System.out.println("Player 2 Hand" + p2.get_PlayHand().toString());
					Bought_Card = true;
				}
			}
	...
	private static void Player1Turn(){
		...
		for(int i = 0; i < p1.Player_Buy_Turns;i++){
			...
			//System.out.println("Player 1: GOLD IN HAND " + Player_1_Gold);
			// Try to buy something
			//System.out.println(Deck.getActionDeck().get(rand).get(0).getCost());
