...
class Player{
	...
	public void PayPiper(int cost){
		...
		System.out.println(Player_Discard.toString());
	}
	...
	public void newHand(){
		// Discard the rest of the player hand  stupid thing wouldn't work
	...
	private void handle_Embargo(){
		...
		while (safe == false){
			...
			System.out.println("This is the randomVal " + randomVal);
		}
	...
	private void handle_CutPurse(Player p){
		System.out.println(this.Player_Hand.toString());
		for(int i = 0; i < Player_Hand.size(); i++){
			if (Player_Hand.get(i).getName() == "Copper"){
				Player_Hand.remove(i); //get rid of a copper 
				System.out.println(this.Player_Hand.toString());
				return; 
			}
		}
	...
	//This will always add to p2 BUG
	private void handle_Ambassador(Player p1, Player p2){
		...
		for(int i = 0; i < Deck.getActionDeck().size(); i++){
			if (Deck.getActionDeck().get(i).size() != 0){
				if (Deck.getActionDeck().get(i).get(0).toString().equals(RandCard.toString())){
		...
		for(int i = 0; i < Deck.getVCDeck().size(); i++){
			if (Deck.getVCDeck().get(i).size() != 0){
				if (Deck.getVCDeck().get(i).get(0).toString().equals(RandCard.toString())){
		...
		String test = "Copper";
		for(int i = 0; i < Deck.getMoneyDeck().size(); i++){
			if (Deck.getMoneyDeck().get(i).size() != 0)
				test = Deck.getMoneyDeck().get(i).get(0).toString();
			String test2 = RandCard.toString();
			if (test.equals(test2) && Deck.getMoneyDeck().get(i).size() != 0){
